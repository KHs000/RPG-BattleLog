...
public class NPC {
    ...
    public NPC () throws IOException{
        ...
        if (obj.Exists() && obj.checaPath("Skyrim-NPCs")) {local = obj.SelecionaPath("Skyrim-NPCs");}
        else {
            ...
            if (path.showOpenDialog(path) == JFileChooser.APPROVE_OPTION) {
                ...
                obj.SalvaConfiguracoes(local, "Skyrim-NPCs");
            }
            else {local = null;}
        }
    }
    ...
    public void Cria () throws IOException {
        if (npcFile.exists() == false) {
           npcFile.createNewFile();
        }
    }
    
    public void gravaFicha () throws IOException {
        npcFile = new File (local + Name + ".txt");
        if (true) {
            try (BufferedWriter dado = new BufferedWriter (new FileWriter (npcFile))){
                dado.write("Nome:");
                dado.write(Name);
                dado.newLine();
                dado.write("HP:");
